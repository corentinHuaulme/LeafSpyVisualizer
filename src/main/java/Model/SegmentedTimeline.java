...
* Foundation, Inc., 5Franklin Street, Fifth Floor, Boston, MA  02110-1301,
* USA.
*
...
*
* $Id: SegmentedTimeline.java,v 1.9.2.2007/02/04mungady Exp $
*
* Changes
* -------
* 23-May-200: Version (BK);
* 15-Aug-200: Implemented Cloneable (DG);
* 01-Jun-200: Modified to compile with JDK 1.2.(DG);
* 30-Sep-200: Replaced getTime().getTime() with getTimeInMillis() (DG);
* 04-Nov-200: Reverted change of 30-Sep-2004, won't work with JDK 1.(DG);
* 11-Jan-200: Removed deprecated code in preparation for 1.0.release (DG);
* ------------- JFREECHART 1.0.x ---------------------------------------------
* 14-Nov-200: Fix in toTimelineValue(long) to avoid stack overflow (DG);
* 02-Feb-200: Removed author tags all over JFreeChart sources (DG);
*
*/
...
* just the number of milliseconds since January 1, 1970, 0GMT as
...
* daily segments, where there are included (Monday through Friday) and 2
...
* relative to January 1, 1970, 0GMT or any other date. This creates a
...
* 0GMT of the first Monday after January 1, 1970. In this class, the
* constant FIRST_MONDAY_AFTER_190refers to a reference point of the first
...
* application, where the trading period is defined as 9:0AM through 4:0PM
...
* that maps the 9:0AM to 4:0PM period. Because the baseTimeline defines a
* timeline of Monday through Friday, the resulting (combined) timeline will
* expose the period 9:0AM through 4:0PM only on Monday through Friday,
...
public class SegmentedTimeline implements Timeline, Cloneable, Serializable {
    ...
    static {
        ...
        // calculate midnight of first monday after 1/1/190relative to
        // current locale
        ...
        // FIRST_MONDAY_AFTER_190= cal.getTime().getTime();
    ...
    /**
     * Constructs a new segmented timeline.
     */
    public SegmentedTimeline(){
        this.segmentList = new ArrayList<>();
        this.groupSegmentCount = 0;
    }

    public void addSegment(Date start){
        Date end = start;
        end.setDate(start.getDate()+1);
        this.segmentList.add(new Segment(start.getTime(),end.getTime()));
        this.startTime = this.segmentList.get(0).getSegmentStart();
        this.groupSegmentCount++;
    }

    public int getSegmentNumber(long millis){
        int res = 0;
        for(Segment s : this.segmentList){
            if(s.getSegmentStart() >= millis && s.getSegmentEnd() <= millis){
                return res;
            }
            res++;
        }
    ...
    /**
     * Factory method to create a 15-min, 9:0AM thought 4:0PM, Monday
     * through Friday SegmentedTimeline.
     * <P>
     * This timeline uses a segmentSize of FIFTEEN_MIN_SEGMENT_SIZE. The
     * segment group is defined as 2included segments (9:0AM through
     * 4:0PM) and 6excluded segments (4:0PM through 9:0AM the next day).
     * <P>
     * In order to exclude Saturdays and Sundays it uses a baseTimeline that
     * only includes Monday through Friday days.
     * <P>
     * The <code>startTime</code> of the resulting timeline will be 9:0AM
     * after the startTime of the baseTimeline. This will correspond to 9:0AM
     * of the first Monday after 1/1/1900.
     *
     * @return A fully initialized SegmentedTimeline.
     */
    public static SegmentedTimeline newFifteenMinuteTimeline() {
        ...
        timeline.setStartTime(
                FIRST_MONDAY_AFTER_1900+ 3* timeline.getSegmentSize()
        );
    ...
    public boolean equals(Object o) {
        if (o instanceof SegmentedTimeline) {
            ...
            boolean b= (this.segmentSize == other.getSegmentSize());
    ...
    public int hashCode() {
        ...
        result = 3* result + (int) (this.startTime ^ (this.startTime >>> 32));
        return result;
    }
     ...
     *         that this guarantees that the return value will be &gt;= if
    ...
    public class Segment implements Comparable, Cloneable, Serializable {
        ...
        protected Segment(long start, long end){
            ...
            this.segmentNumber = calculateSegmentNumber(this.millisecond);
        }
        ...
        public long calculateSegmentNumber(long millis) {
           return  SegmentedTimeline.this.getSegmentNumber(millis);
        }
